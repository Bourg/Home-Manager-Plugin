			...
			if (args.length == 1)
			{
				...
				if (player != null)
				{
					...
					if (plugin.selectedLocations.get(player) != null && plugin.selectedLocations.get(player)[0] != null && plugin.selectedLocations.get(player)[1] != null)
					{
						...
						if (plugin.regions == null || plugin.regions.size() == 0 || !(new Region(plugin.selectedLocations.get(player)[0], plugin.selectedLocations.get(player)[1]).overlap(plugin.regions)))
						{
							sender.sendMessage(plugin.selectedLocations.get(player)[0] == null ? "null" : "not null");
							sender.sendMessage(plugin.selectedLocations.get(player)[1] == null ? "null" : "not null");
							
							plugin.regions.add(new Region(plugin.selectedLocations.get(player)[0], plugin.selectedLocations.get(player)[1], args[0], player));
